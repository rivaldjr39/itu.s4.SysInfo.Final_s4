...
<?php /*
  Si tu n'as pas de layout('layout/client'), retire cette ligne et
...
<style>
    ...
    .transfert-item.envoye .sens-icone {
        background: rgba(193, 69, 43, 0.1);
        color: var(--error);
    }

    .transfert-item.recu .sens-icone {
    ...
    .transfert-details .ligne-principale {
        display: flex;
        justify-content: space-between;
    ...
    .transfert-item.envoye .montant {
        color: var(--error);
    }

    .transfert-item.recu .montant {
        color: var(--success);
    }

    .transfert-details .meta {
        display: flex;
        justify-content: space-between;
        margin-top: 4px;
        font-size: 12px;
...
<div class="historique-page">
    <div class="historique-card">
        ...
        <div class="historique-body">
            <a href="<?= site_url('transfert') ?>" class="btn-nouveau">Faire un nouveau transfert</a>

            <?php if (empty($historique)): ?>
                <div class="etat-vide">
                    <div class="titre-vide">Aucun transfert pour l'instant</div>
                    <p>Chaque transfert que tu envoies ou reçois apparaîtra ici.</p>
                </div>
            <?php else: ?>
                <div class="liste-transferts">
                    <?php foreach ($historique as $t): ?>
                        <?php
                            $estEnvoye = (int) $t['source_client'] === (int) $client_id;
                            $sens = $estEnvoye ? 'envoye' : 'recu';
                            $autrePartie = $estEnvoye
                                ? ($t['numero_destination'] ?? 'Numéro inconnu')
                                : ($t['numero_source'] ?? 'Numéro inconnu');
                            $signe = $estEnvoye ? '-' : '+';
                            $montantAffiche = $estEnvoye ? $t['montant_total'] : $t['montant'];
                        ?>
                        <div class="transfert-item <?= $sens ?>">
                            <div class="sens-icone"><?= $estEnvoye ? '↑' : '↓' ?></div>
                            <div class="transfert-details">
                                <div class="ligne-principale">
                                    <span class="libelle"><?= $estEnvoye ? 'Envoyé à ' : 'Reçu de ' ?><?= esc($autrePartie) ?></span>
                                    <span class="montant"><?= $signe ?><?= number_format((float) $montantAffiche, 0, ',', ' ') ?> Ar</span>
                                </div>
                                <div class="meta">
